...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const bottomNav = nav.filter((item) =>
    ["/icms/dashboard", "/icms/transactions/new", "/icms/scan", "/icms/transactions", "/icms/incidents"].includes(item.href)
  );

  return (
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-40 border-b bg-card/95 backdrop-blur">
        <div className="h-0.5 bg-gradient-to-r from-icms-brand via-primary to-brand" />
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-2 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/home"
              className="hidden rounded-md px-2 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground sm:inline-flex"
            >
              AVSEC Reports
            </Link>
        ...
        <div className="grid auto-cols-fr grid-flow-col">
          {bottomNav.map((item) => (
            <Link
              ...
              href={item.href}
              className={
                item.href === "/icms/scan" || item.href === "/icms/transactions/new"
            ...
            >
              <item.icon
                className={
                  item.href === "/icms/scan" || item.href === "/icms/transactions/new" ? "h-6 w-6" : "h-5 w-5"
                }
              />
              {item.label.split(" ")[0]}
            </Link>
